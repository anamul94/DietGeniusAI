...
from app.constants.prompts import memory_instruction
...
embeddings = ModelProvider.bedrock_embeddings(id=models.TITAN_EMBED_TEXT)
conn = Connection.connect(settings.DATABASE_URL, **connection_kwargs)
store = PostgresStore(conn=conn, index={"embed": embeddings, "dims": 768})
pg_store = store.setup()
